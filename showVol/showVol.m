...
% AMD ToDos
%   allow cell array of volumes and meshes - done
%   allow RGB volumes (24-bit color) - done
%   allow for display of pre-rendered images
%   change contrast control to standard window / level
%   allow segmentations
%     list of numbered label names and colors for each
%     mutually exclusive labels, interger per voxel
%     partial membership (floating point value per label -- possibly sparse)
...
if isfield(handles,'debug') && handles.debug
  PrintStack(dbstack)
end
...
%handles.debug = true; % AMD: enable debugging code
handles.debug = false; % AMD: disable debugging code
...
handles = parse_arguments(handles,varargin);

% ToDos AMD
%   Handle different data types:
%     3- or 4-d volumes
%     individual ctx_vols
%     cell arrays of 3- or 4-d volumes
%     cell arrays of ctx_vols
%     individual surface structs
%     cell arrays of surface structs
%   Make function to handle each element of array
...
if 1 % (handles.numVols == 1) || (sum(range(voxNumCheck)) < eps)
...
for vvv = 1:handles.numVols
  if isfield(handles.vols{vvv}, 'crange')
    minI(vvv) = handles.vols{vvv}.crange(1);
    maxI(vvv) = handles.vols{vvv}.crange(2);
  end
end
handles.maxI = maxI;
handles.minI = minI;
...
if ~isempty(handles.initialRCS)
  handles.initialLPH = inv(Mlph2vxl) * colvec(handles.initialRCS);                      
end

if ~isempty(handles.initialLPH)
  handles = gotoPointLPH(handles, handles.initialLPH);
  printInfo(handles);
  guidata(hObject, handles);
end
...
%% ------------------------------------------------------------
%% --- Function to parse optional command line arguments
%% ------------------------------------------------------------
function handles = parse_arguments(handles,args)

if isfield(handles,'debug') && handles.debug
  PrintStack(dbstack)
end

for iii = 1:length(args)
  v = args{iii};
  if iscell(v)
    for vi = 1:length(v)
      handles = parse_arguments(handles,v(vi));
    end
  elseif isstruct(v) 
    if (isfield(v,'imgs')) && (isfield(v,'Mvxl2lph')) % volStruct
      handles.numVols = handles.numVols + 1;
      handles.vols{handles.numVols} = v;
    end
    if isfield(v,'vertices') % meshStruct
      handles.numSurfs = handles.numSurfs + 1;
      handles.surfs(handles.numSurfs).vertices = v.vertices;
      handles.surfs(handles.numSurfs).faces = v.faces;
      handles.surfs(handles.numSurfs).color = [1 1 0];
      handles.surfs(handles.numSurfs).linewidth = 1;
      handles.surfs(handles.numSurfs).linestyle = '-';
      if isfield(v,'color')
        if ~isempty(v.color)
          handles.surfs(handles.numSurfs).color = v.color;
        end
      end
      if isfield(v, 'linewidth') && ~isempty(v.linewidth)
        handles.surfs(handles.numSurfs).linewidth = v.linewidth;
      end
      if isfield(v, 'linestyle') && ~isempty(v.linestyle)
        handles.surfs(handles.numSurfs).linestyle = v.linestyle;
      end
    end
    if isfield(v,'RCS') % Should also handle RAS, LPH
      handles.initialRCS = v.RCS; handles.initialRCS(4) = 1;
    end
  else
    for vi = 1:size(v,4)
      handles.numVols = handles.numVols + 1;
...
colormap(gray);
if isfield(handles.vols{handles.currentVol},'imgs')
  if size(handles.vols{handles.currentVol}.imgs,4)==3 % RGB image?
    ima = squeeze(handles.vols{handles.currentVol}.imgs(:,:,handles.ss,:));
    handles.imCR = image(ima); 
  elseif size(handles.vols{handles.currentVol}.imgs,4)==1
    ima = squeeze(handles.vols{handles.currentVol}.imgs(:,:,handles.ss));
    handles.imCR = imagesc(ima,handles.CLim(handles.currentVol, :)); 
  end
  if isfield(handles.vols{handles.currentVol},'cmap')
    colormap(handles.vols{handles.currentVol}.cmap);
  end
...
handles.CtextCR = text(handles.cc, 1, 'S');
handles.RtextCR = text(1, handles.rr, 'R');
handles.ClineCR = line(size(ima,2)/handles.ccMax*[handles.cc handles.cc],size(ima,1)/handles.rrMax*[0.5 0.5+handles.rrMax]);
handles.RlineCR = line(size(ima,2)/handles.ccMax*[0.5 0.5+handles.ccMax],size(ima,1)/handles.rrMax*[handles.rr handles.rr]);

%% Draw in axis SR, which draws imgs(:,j,:)
axes(handles.axesSR);
colormap(gray);
if isfield(handles.vols{handles.currentVol},'imgs')
  if size(handles.vols{handles.currentVol}.imgs,4)==3
    ima = squeeze(handles.vols{handles.currentVol}.imgs(:,handles.cc,:,:));
    handles.imSR = image(ima); 
  else
    ima = squeeze(handles.vols{handles.currentVol}.imgs(:,handles.cc,:));
    handles.imSR = imagesc(ima,handles.CLim(handles.currentVol, :)); 
  end
  if isfield(handles.vols{handles.currentVol},'cmap')
    colormap(handles.vols{handles.currentVol}.cmap);
  end
...
handles.RtextSR = text(1, handles.rr, 'P');
handles.SlineSR = line(size(ima,2)/handles.ssMax*[handles.ss handles.ss],size(ima,1)/handles.rrMax*[0.5 0.5+handles.rrMax]);
handles.RlineSR = line(size(ima,2)/handles.ssMax*[0.5 0.5+handles.ssMax],size(ima,1)/handles.rrMax*[handles.rr handles.rr]);
...
if isfield(handles.vols{handles.currentVol},'imgs')
  if size(handles.vols{handles.currentVol}.imgs,4)==3
    ima = permute(squeeze(handles.vols{handles.currentVol}.imgs(handles.rr,:,:,:)),[2 1 3]);
    handles.imCS = image(ima);
  else
    ima = squeeze(handles.vols{handles.currentVol}.imgs(handles.rr,:,:))';
    handles.imCS = imagesc(ima,handles.CLim(handles.currentVol, :)); 
  end
  if isfield(handles.vols{handles.currentVol},'cmap')
    colormap(handles.vols{handles.currentVol}.cmap);
  end
elseif isfield(handles.vols{handles.currentVol},'imgs1')
  ima = handles.vols{handles.currentVol}.imgs1{handles.rr};
  handles.imCS = image(ima);
end
axis image; axis xy
set(handles.axesCS, 'DataAspectRatio', [handles.voxSize(3) handles.voxSize(2) 1]);
handles.CtextCS = text(handles.cc, handles.ssMax, 'A');
handles.StextCS = text(1, handles.ss, 'R');
handles.ClineCS = line(size(ima,2)/handles.ccMax*[handles.cc handles.cc],size(ima,1)/handles.ssMax*[0.5 0.5+handles.ssMax]);
handles.SlineCS = line(size(ima,2)/handles.ccMax*[0.5 0.5+handles.ccMax],size(ima,1)/handles.ssMax*[handles.ss handles.ss]);
...
for mmm = 1:handles.numSurfs
  ...
  %% Draw in axesCR, which draws imgs(:,:,k)
  axes(handles.axesCR);
  [startPts, endPts] = findContour(handles.surfs(mmm),handles.surfs(mmm).vertices(:,3),handles.ss);

  startR = startPts(:,1); endR = endPts(:,1);
  startC = startPts(:,2); endC = endPts(:,2);

  hold on; 
  lh = quiver(startC, startR, endC-startC, endR-startR, 0, handles.surfs(mmm).linestyle); % Why use quiver?
  hold off
  
  set(lh, 'Color', handles.surfs(mmm).color);
  set(lh, 'LineWidth', handles.surfs(mmm).linewidth);
...
for mmm = 1:handles.numSurfs
  ...
  hold on; 
  lh = quiver(startS, startR, endS-startS, endR-startR, 0, handles.surfs(mmm).linestyle); 
  hold off
...
for mmm = 1:handles.numSurfs
  ...
  hold on; 
  lh = quiver(startC, startS, endC-startC, endS-startS, 0, handles.surfs(mmm).linestyle); 
  hold off
...
elseif isfield(handles.vols{handles.currentVol},'imgs1')
%  ima = handles.vols{handles.currentVol}.imgs1{handles.ss}; % Should scale to size of baseline image
  ima = handles.vols{handles.currentVol}.imgs3{handles.ss};
end
...
elseif isfield(handles.vols{handles.currentVol},'imgs1')
  ima = handles.vols{handles.currentVol}.imgs3{handles.rr};
end
...
elseif isfield(handles.vols{handles.currentVol},'imgs1')
  ima = handles.vols{handles.currentVol}.imgs3{handles.ss};
  set(handles.axesCR, 'Xlim', size(ima,2)/handles.ccMax*handles.ccLim); sfc = size(ima,2)/handles.ccMax;
  set(handles.axesCR, 'Ylim', size(ima,1)/handles.rrMax*handles.rrLim); sfr = size(ima,2)/handles.rrMax;
  ima = handles.vols{handles.currentVol}.imgs2{handles.cc};
  set(handles.axesSR, 'Xlim', size(ima,2)/handles.ssMax*handles.ssLim);
  set(handles.axesSR, 'Ylim', size(ima,1)/handles.rrMax*handles.rrLim); sfs = size(ima,2)/handles.ssMax;
  ima = handles.vols{handles.currentVol}.imgs1{handles.rr};
  set(handles.axesCS, 'Xlim', size(ima,2)/handles.ccMax*handles.ccLim);
  set(handles.axesCS, 'Ylim', size(ima,1)/handles.ssMax*handles.ssLim);

  set(handles.CtextCR, 'Position', [sfc*handles.cc sfr*handles.rrLim(1)]);
  set(handles.RtextCR, 'Position', [sfc*handles.ccLim(1) sfr*handles.rr]);
  set(handles.StextSR, 'Position', [sfs*handles.ss sfr*handles.rrLim(1)]);
  set(handles.RtextSR, 'Position', [sfs*handles.ssLim(1) sfr*handles.rr]);
  set(handles.CtextCS, 'Position', [sfc*handles.cc sfs*handles.ssLim(2)]);
  set(handles.StextCS, 'Position', [sfc*handles.ccLim(1) sfs*handles.ss]);
...
%% ------------------------------------------------------------ 
function handles = updateDisplay_newvol(handles)
...
if isfield(handles,'debug') && handles.debug
  fprintf(1,'displayNewOrientation\n');
  PrintStack(dbstack)
end
...
handles = updateDisplay_newvol(handles); % This calls newDisplay again
...
if isfield(handles,'debug') && handles.debug
  fprintf(1,'decrementColumnNumber\n');
  PrintStack(dbstack)
end
...
if isfield(handles,'debug') && handles.debug
  fprintf(1,'incrementColumnNumber\n');
  PrintStack(dbstack)
end
...
if isfield(handles,'debug') && handles.debug
  fprintf(1,'decrementSliceNumber\n');
  PrintStack(dbstack)
end
...
if isfield(handles,'debug') && handles.debug
  fprintf(1,'incrementSliceNumber\n');
  PrintStack(dbstack)
end
...
if chosenVol == handles.currentVol
  if 0 % sum(handles.hideVol) < handles.numVols-1 % Disable hiding volume (perhaps enable, using modifier keys)
...
set(handles.figure, 'Units', 'characters');
%fprintf(1,'Position in characters: %s\n',num2str(round(get(handles.figure,'position')),'%d '));
set(handles.figure, 'Units', 'points');
%fprintf(1,'Position in points: %s\n',num2str(round(get(handles.figure,'position')),'%d '));
%disp(get(handles.figure,'position'))
